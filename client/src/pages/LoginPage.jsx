...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.errors) setError(data.errors[0]?.msg || 'Please check your login details.');
      else if (data?.error) setError(data.error);
      else setError('Could not reach the server. Make sure the backend is running on port 5000 and MongoDB is connected.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-md animate-fade-in rounded-[28px] border border-white/45 bg-white/70 p-8 shadow-2xl shadow-slate-900/10 backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/60">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 pl-10 text-sm font-medium text-slate-950 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:border-white/10 dark:bg-white/10 dark:text-white dark:placeholder:text-slate-400"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 pl-10 pr-10 text-sm font-medium text-slate-950 placeholder:text-slate-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:border-white/10 dark:bg-white/10 dark:text-white dark:placeholder:text-slate-400"
              />
              <button
                type="button"
        ...
        */}
        ...
        <p className="mt-6 text-center text-sm font-medium text-slate-600 dark:text-slate-300">
          ...
          <Link to="/register" className="font-black text-cyan-700 hover:underline dark:text-cyan-300">
            Create one
          </Link>
        </p>
      </div>
